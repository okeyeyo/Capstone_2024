...
public interface FoodApiService {
    ...
    Call<Food> searchnameFood(@Path(value = "name" , encoded = true)
                                    String name, @Query("apikey") String apikey);
    @GET("1/100/RCP_PARTS_DTLS={ingredients}") // 재료로 호출
    Call<Food> searchingredientsFoods(@Path(value = "ingredients", encoded = true)
                                            String ingredients, @Query("apiKey") String apiKey);
    @GET("1/100")
    Call<Food> getAllFoods(@Query("api_key") String apiKey);
}
